...
public class CreateByUserCommandHandler : IRequestHandler<CreateByUserCommand, IResponse>
{
    ...
    public CreateByUserCommandHandler(IPermissionUnitOfWork permissionUnitOfWork, IPermission permission,
    ...
    public async Task<IResponse> Handle(CreateByUserCommand request, CancellationToken cancellationToken)
    {
        var transaction = await _permissionUnitOfWork.UserRepository.BeginTransactionAsync(cancellationToken);

        try
        {
            var (_, user) = _token.GetTokenInformationFromHeader(cancellationToken);
            ...
            var (addedUser, addedLogin, addedToken) = await CreateUser(user.EmailAddress, request, cancellationToken);
    ...
    private async Task<(Domain.Entities.Permission.User user, Login login, Token token)> CreateUser(string userEmail,
        CreateByUserCommand request, CancellationToken cancellationToken)
    {
